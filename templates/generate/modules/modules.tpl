...
import (
	"errors"
	{{if .HasUniqueIndex}}"github.com/jinzhu/gorm"{{end}}
	"{{.PackagePrefix}}/log"
	{{if .HasTime}}"time"{{end}}
)
...
func (p *{{ $.Struct }}) Get_by{{ range $ii, $vv := $v.Columns }}_{{$vv.ColumnName}}{{end}}({{ range $ii, $vv := $v.Columns }}{{if gt $ii 0}}, {{end}}{{$vv.LowerColumn}} {{$vv.ColumnType}}{{end}}) (*{{ $.Struct }}, error) {
	record := new({{ $.Struct }})
	db := DbInstance.Debug()
...
// 分页查询记录
func (p *{{ .Struct }}) Page(queryParams map[string]interface{}) ([]{{ .Struct }}, int, error) {
	records := []{{ .Struct }}{}
	db := DbInstance.Debug()
	...
	notFound := db.Where(queryParams).Find(&records).RecordNotFound()
	if notFound {
		return []{{ .Struct }}{}, 0, nil
	}
