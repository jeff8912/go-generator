...
var (
	...
	{{ $.LowerCamelStruct }}Mapper  = module.{{ .Struct }}Mapper
)
...
func (p *{{ .Struct }}ServiceImpl) Create({{ $.LowerCamelStruct }}Dto *dto.{{ .Struct }}) (res *dto.BaseResult) {
	...
	{{ $.LowerCamelStruct }}Db, err {{if eq $i 0}}:{{end}}= {{ $.LowerCamelStruct}}Mapper.GetBy{{ range $ii, $vv := $v.Columns }}{{$vv.UpperColumn}}{{end}}({{ range $ii, $vv := $v.Columns }}{{if gt $ii 0}}, {{end}}{{ $.LowerCamelStruct }}Dto.{{$vv.UpperColumn}}{{end}})
	if err != nil {
	...
	{{ .LowerCamelStruct }} := module.{{ .Struct }}{ {{ range $i, $v := .Columns }}{{if eq $i 0}}{{else}}
